...
struct Clock
{
    ...
    string to_str(){
        ...
        return change_time(hour) + ":" + change_time(minute) +  ":" + change_time(second);
    }
    void  shift(int diff_second){
        int current = hour * 3600 + minute * 60 + second, changed_time;
        //current - diff_secondが-かどうか判別
        if (0 >= current + diff_second){
            current = 86400;
        }
        changed_time = current + diff_second;

        hour = changed_time / 3600;
        minute = (changed_time % 3600) / 60;
        second = ((changed_time % 3600) % 60);
        if(hour == 24)hour = 0;
